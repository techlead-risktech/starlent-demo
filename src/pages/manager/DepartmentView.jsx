...
import { progressByDepartment } from '../../data/mockReports.js';
...
export default function DepartmentView() {
  ...
  const stats=progressByDepartment.find(d=>d.department===user?.department);
  ...
  return (
    <AdminLayout title="Quản lý phòng ban">
      ...
      {tab==='overview'&&<>
        ...
        :members.map(m=><div key={m.id} className="card" style={{marginBottom:8}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><div style={{display:'flex',alignItems:'center',gap:12}}><div className="avatar">{m.name.charAt(0)}</div><div><div style={{fontWeight:700}}>{m.name}</div><div style={{fontSize:12,color:'var(--color-text-muted)'}}>Cấp {Math.floor(m.xp/500)+1} · 🔥{m.streak} · {m.xp} XP</div></div></div><button className="btn btn--ghost btn--sm" onClick={()=>showToast('📬 Đã gửi nhắc nhở (mock)')}>📬 Nhắc</button></div></div>)}
      </>}

      {tab==='members'&&<div className="table-wrapper"><table className="table"><thead><tr><th>Tên</th><th>Cấp</th><th>XP</th><th>Streak</th><th>Tham gia</th><th>Hành động</th></tr></thead><tbody>{members.map(m=><tr key={m.id}><td><strong>{m.name}</strong></td><td>{Math.floor(m.xp/500)+1}</td><td>{m.xp}</td><td>🔥{m.streak}</td><td>{m.joinedAt}</td><td><button className="btn btn--ghost btn--sm" onClick={()=>showToast('📬 Đã gửi')}>📬</button></td></tr>)}</tbody></table></div>}

      {tab==='reminders'&&<div>
        <div className="card" style={{marginBottom:16}}>
          <div className="input-group" style={{marginBottom:12}}><label className="input-label">Người nhận</label><select className="input">{members.map(m=><option key={m.id}>{m.name}</option>)}</select></div>
          <div className="input-group" style={{marginBottom:12}}><label className="input-label">Nội dung</label><textarea className="input" rows={3} placeholder="Nhập nội dung nhắc nhở..."/></div>
          <button className="btn btn--primary btn--full" onClick={()=>showToast('📬 Đã gửi nhắc nhở (mock)')}>📬 Gửi nhắc nhở</button>
        </div>
