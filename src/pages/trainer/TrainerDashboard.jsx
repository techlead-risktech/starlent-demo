...
export default function TrainerDashboard() {
  ...
  return (
    <AdminLayout title="Giảng viên">
      ...
      {tab === 'learners' && <div className="table-wrapper"><table className="table"><thead><tr><th>Tên</th><th>Phòng ban</th><th>Khoá học</th><th>Điểm</th><th>Streak</th><th>HĐ cuối</th></tr></thead><tbody>{data.learnerProgressDetail.map((l) => <tr key={l.userId}><td><strong>{l.userName}</strong></td><td>{l.department}</td><td>{l.coursesCompleted}/{l.coursesEnrolled}</td><td>{l.avgScore}</td><td>🔥{l.streak}</td><td>{l.lastActive}</td></tr>)}</tbody></table></div>}
