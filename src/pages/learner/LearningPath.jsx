...
export default function LearningPath() {
  ...
  const completedItems = ls?.completedItems || [];
  ...
  const withStatus = sorted.map(c => ({ course: c, progress: getCourseProgress(c, completedItems) }));
  ...
  const suggested = courses.filter(c => {
    if (getCourseProgress(c, completedItems) > 0) return false;
          ...
          {suggested.length > 0 && (
            <div style={{marginTop:24}}>
              ...
              <div className="grid-2">
                {suggested.map(c => (
                  <div key={c.id} className="card card--hoverable" onClick={()=>navigate(`/learner/course/${c.id}`)}>
                    ...
                    <div style={{fontSize:12,color:'var(--color-text-muted)',marginTop:8}}>⏱ {c.duration}p · ⭐ {c.rating}</div>
                  </div>
                ))}
              </div>
            </div>
