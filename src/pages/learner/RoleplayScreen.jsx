...
export default function RoleplayScreen() {
  ...
  const [content, setContent] = useState(null); const [recording, setRecording] = useState(false);
  const [recorded, setRecorded] = useState(false); const [submitted, setSubmitted] = useState(false);
  const [recTime, setRecTime] = useState(0); const timerRef = useRef(null);
  ...
  const start=()=>{setRecording(true);setRecTime(0);timerRef.current=setInterval(()=>setRecTime(t=>t+1),1000);};
  const stop=()=>{setRecording(false);setRecorded(true);if(timerRef.current)clearInterval(timerRef.current);};
  const submit=()=>{completeItem(content?.id,20);setSubmitted(true);showToast('✅ Đã gửi! +20 XP');};
