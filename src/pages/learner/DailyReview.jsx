...
export default function DailyReview() {
  ...
  const persistCardReview = async (cardId, rating) => {
    try {
      await saveCardReviewApi({ cardId, rating });
    } catch {
      saveCardReview(cardId, rating);
    }
  };
  ...
  const completeReviewSession = async (count) => {
    ...
    try {
      await completeLearningItem({ itemId: 'daily_review', xpAmount: totalXp });
    } catch {
      completeItem('daily_review', totalXp);
    }
    setXp(totalXp);
    setDone(true);
      ...
      <div style={{ padding: 16 }}>
        ...
        {flipped && (
          <div style={{ marginTop: 20 }}>
            <p style={{ textAlign: 'center', fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Bạn nhớ được bao nhiêu?</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
              {[{ label: 'Quên', key: 'forgot', color: '#EF4444' }, { label: 'Khó', key: 'hard', color: '#F59E0B' }, { label: 'Ổn', key: 'ok', color: '#3B82F6' }, { label: 'Dễ', key: 'easy', color: '#10B981' }].map((button) => (
                <button key={button.key} className="btn" style={{ background: button.color, color: '#fff' }} onClick={() => rate(button.key)}>{button.label}</button>
              ))}
            </div>
          </div>
        )}
