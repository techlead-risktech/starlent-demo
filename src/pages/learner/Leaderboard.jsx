...
function fallbackLeaderboard(currentUserId) {
  const learners = users.filter((u) => u.role === 'learner').sort((a, b) => b.xp - a.xp).map((u, i) => ({ id: u.id, name: u.name, department: u.department, xp: u.xp, streak: u.streak, rank: i + 1 }));
...
export default function Leaderboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!mounted) return;
        setData({ learners: response.learners || [], myRank: response.myRank ?? null });
      } catch {
        if (!mounted) return;
  ...
  return (
    <LearnerLayout topBar={<div className="page__header"><div className="page__title">{t('learnerPages.leaderboard.title')}</div></div>}>
        ...
        {loading ? (
          ...
          <>
            <div style={{ textAlign: 'center', padding: 20, background: 'linear-gradient(135deg,#FFF7ED,#FFF0EB)', borderRadius: 'var(--radius-lg)', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>{t('learnerPages.leaderboard.yourRank')}</div>
              ...
              return (
                <div key={l.id} className="card" style={{ marginBottom: 8, background: isMe ? 'var(--color-primary-light)' : 'var(--color-surface)', borderColor: isMe ? 'var(--color-primary)' : 'var(--color-border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 14 }}>{l.name}{isMe && ` ${t('learnerPages.leaderboard.you')}`}</div>
                      <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>{l.department}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--color-primary)' }}>{f(t('learnerPages.leaderboard.points'), { xp: l.xp.toLocaleString() })}</div>
                      <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>{f(t('learnerPages.leaderboard.level'), { level })} · 🔥{l.streak}</div>
                    </div>
                  </div>
                </div>
              );
